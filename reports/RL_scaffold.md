...
Detailed design rationale and calibration evidence supporting `rl/mdp_schema.md`, the formal MDP spec for
the Aido Rover patrol / anomaly-response task. Both are produced by
`notebooks/W02_Sequence_and_RL_Scaffolding.ipynb` (Section 2), which also generates `rover_transitions.csv`.
Companion to `reports/W02_Rover_World_Core_Design.md` (the world core itself) — that document is the
citation of record for "matches the world-core design" claims below.

---

## 1. World-core suitability

The shared world core (`rl/rover_world.py`, `RoverWorld`) is used as-is for RL, unmodified. Its two-layer
architecture (`world.step` physics core vs. a deferred `env.step` MDP wrapper in W05) is the correct shape
for this task: the physics is validated and frozen, while reward/observation design can keep iterating in
W05 without touching data generation. `raise-alert` and `return-to-base` having no motion effect in the
core is a feature of that split, not a gap — their operational effect (clearing a blockage, ending an
episode) is a wrapper-layer concern.
