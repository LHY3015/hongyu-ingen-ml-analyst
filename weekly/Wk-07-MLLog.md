...
from 91 % of the ≤10 ms breach-detection tier to 25 %. Calibration against the STUM class's
internal target of ECE 0.031: the MLP meets it with no calibration layer (0.025 raw), the
Transformer misses it raw (0.041) and lands just above it after validation-fitted temperature
scaling (0.032); the open calibration problem is cross-block transfer (the threshold swings), not
